...
However, in case MOP is installed after an undeploy process, MOP will delete and recreate
...
=== Backup DB
To back up MOP DB using the tools included in the chart package,
...
[NOTE]
====
The password for the user is the one used when creating the MOP database.
...
[IMPORTANT]
====
The database restore can be performed without stopping MOP. Keep in mind that the restore also results in the loss of existing VNO configurations.
====
