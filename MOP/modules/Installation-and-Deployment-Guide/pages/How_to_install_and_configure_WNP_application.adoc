...
*config.toml* (/etc/containerd/config.toml) must be configured for authentication on every Kubernetes node where you want to download the MOP image for Kubernetes environment with containerd runtime.
...
*crio.conf* (/etc/crio/crio.conf) must be configured for authentication on every Kubernetes node where you want to download the MOP image for Kubernetes environment with crio runtime.
...
. Add the *NOKIA software repository* to your Kubernetes host using the following command. The values of "NOKIALOGIN" and "NOKIAPWD" are automatically set when you execute the NokiaSWRepoAccessProvider binary file.
Run the following command every time after you execute NokiaSWRepoAccessProvider binary file and when you want to check for updates from the Nokia software repository.
+
.Add helm repository
[source]
----
...
----
+
If you are running the above command on helm version {helm-version}, you will need to include the additional argument '--pass-credentials'. Refer to https://github.com/helm/helm/issues/9851 for more details.
...
[NOTE]
====
If a customer is successfully onboarded, but unable to access the MOP software
...
----
+
.. Configure the namespace if needed (it must be the same where Altiplano is running),
the "release_name" parameter to "mop" value and the "install_mop" parameter to true in the tools.conf file :
+
[IMPORTANT]
The release_name and the install_mop parameters within the tools.conf file determines the installation mode for the platform.
To install it in MOP mode, the release_name must be set to "mop" and install_mop to true prior to running the installation process.
+
[source, bash]
----
...
[NOTE]
====
If you have created an INP user on {main-base-app} keycloak, then you can Log in to the system using username and password.
====


==== MOP Mode verification step

To verify that MOP mode has been correctly installed, perform the following steps:
...
. Create an ISP and a user for the ISP

. Log in to the MOP application and verify that the 'Intents' menu includes the following sections for managing the OLT intent stack:

image::OLTStackIntentSection_ISP.png[align="center"]

[NOTE]
====
All the supported intent types must be shown under each specific category shown under the "Intents" menu
====
