...
----
+
.. Configure the namespace if needed and the release name to "mop" value in the tools.conf file (it must be the same where Altiplano is running):
+
[IMPORTANT]
"The release_name parameter within the tools.conf file determines the installation mode for the platform. To install it in MOP mode, the parameter must be set to release_name="mop" prior to running the installation process."
+
[source, bash]
----
