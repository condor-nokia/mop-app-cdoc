...
----
+
.. Configure the namespace if needed, the "release_name" parameter to "mop" value and the "install_mop" parameter to true in the tools.conf file (it must be the same where Altiplano is running):
+
[IMPORTANT]
...
. Login to MOP application and check that the "Intents" menu contains the following sections that allows to manage the whole
OLT intent stack:
